...
function EditLoadForm({ load, onCancel, onUpdate }) {
  ...
  const [cargoEdits, setCargoEdits] = useState({}); // cargoId → editable fields

  useEffect(() => {
    const fetchCargo = async () => {
      try {
        const res = await fetch(`/api/cargo/${load.loadassignmentId}`);
        const data = await res.json();
        setCargoList(data);

        // Initialize cargo edit state
        const editState = {};
        data.forEach((cargo) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Update load
      const res = await fetch(`/api/loads/${load.loadassignmentId}`, {
        method: "PUT",
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to update load.");

      // Update each cargo
      for (const cargoId of Object.keys(cargoEdits)) {
        const cargoData = cargoEdits[cargoId];
        const resCargo = await fetch(`/api/cargo/${cargoId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(cargoData),
        });
        if (!resCargo.ok) {
          const err = await resCargo.json();
          console.error("Error updating cargo ID:", cargoId, err);
        }
      }
