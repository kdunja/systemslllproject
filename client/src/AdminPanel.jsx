...
function AdminPanel() {
  const [tab, setTab] = useState("users"); // users | loads | ratings | messages | dashboard
  ...
  const toggleUserActive = async (user) => {
    ...
    try {
      await axios.patch(`/api/admin/users/${user.userId}/status`, { is_active: next });
      fetchUsers();
    } catch (err) {
  ...
  const fetchLoads = async () => {
    try {
      const res = await axios.get("/api/admin/loads");
      setLoads(res.data);
    } catch (err) {
  ...
  const handleDeleteLoad = async (loadId) => {
    ...
    try {
      await axios.delete(`/api/admin/loads/${loadId}`);
  ...
  const fetchRatings = async () => {
    try {
      ...
      const res = await axios.get(`/api/admin/ratings${query}`);
      setRatings(res.data);
    } catch (err) {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const res = await axios.get(`/api/admin/messages${query}`);
  ...
  // Load tab data on switch / first render
  useEffect(() => {
  ...
  const maxStatusCount =
    stats && stats.statusCounts.length
        ...
        <>
          {!stats ? (
            ...
            <>
              ...
              {/* Simple CSS bars instead of Recharts */}
              <div style={{ marginTop: 24, background: "#252422", borderRadius: 8, padding: 16 }}>
                ...
                {stats.statusCounts && stats.statusCounts.map((row) => {
                  ...
                  return (
                    <div key={row.status} style={{ marginBottom: 10 }}>
                      <div style={{ marginBottom: 4, fontSize: 14 }}>{row.status} ({count})</div>
